...
import {
  ...
  useScroll,
  useTransform,
} from "framer-motion"; // project ships framer-motion@11; same API as "motion/react"
...
export const HeroButton = React.forwardRef<HTMLButtonElement, HTMLMotionProps<"button">>(
  ({ className, transition, ...props }, ref) => (
    <motion.button
      ...
      ref={ref}
      className={cn(
        "group relative flex w-fit items-center rounded-full border border-accent bg-white/10 px-5 py-2.5 text-white font-semibold shadow-[0px_4px_24px_rgba(255,106,26,0.55)] transition-colors hover:bg-accent",
        className,
      )}
      {...props}
    />
